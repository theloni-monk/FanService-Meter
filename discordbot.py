...
async def isthishentai(ctx):
    async with ctx.typing():
        ...
        cvimg = cv2.cvtColor(cv2.resize(img, ai.img_size),
                             cv2.COLOR_BGR2GRAY)  # reduces dimention
        # we need to get it back to 3 channels
        cvimg = cv2.cvtColor(cvimg, cv2.COLOR_GRAY2BGR)
        score, label = ai.test_img(cvimg)
        if score > 60:
            # TODO: make it call the user a weeb if they send hentai
            await ctx.send("The AI is %.1f%% sure that the image analyzed is %s" % (score, label))
        else:
            await ctx.send("The AI is unsure as to the content of the image analyzed, but it might be %s" % label)
