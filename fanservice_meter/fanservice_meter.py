...
class fsm_ai:
    """Pre-built CNN for destinguishing between sfw and pornographic manga"""
    ...
    def __init__(self, img_size=(300, 300), batch_size=8, use_premade=True, make_new=False):
        ...
        model_path = os.path.join(path_prefix, 'wholemodel.h5')
        self._model = keras.models.load_model(
            model_path) if use_premade else None
        try:
            assert not (use_premade and make_new) #xor
        except AssertionError:
            raise ValueError(
                "cannot both use premade model and generate new model")
        if make_new:
    ...
    def _make_model(self):
        ...
        x = layers.experimental.preprocessing.Rescaling(
            1.0 / 255)(inputs)  # scale rgb to float
    ...
    def load(self, filename):
        try:
            ...
            raise TypeError(
                "Attempted to load file that was not of format keras h5")
    ...
    def test_img(self, img_in, **kwargs):
        ...
        # convert from 0->1 to 0-100% confidence per label
        score = (1 - score) if 1-score > 0.5 else score
        score -= 0.5
        score *= 200
